...
def prepare_balanced_dataset(full_dataset, study):
    X, y = prepare_dataset(full_dataset, study)
    X,y  = random_upsample_balance(X,y)
    X,y  = shuffle(X,y)
    return X,y
...
for study in sorted(all_studies):
    X_full, y_full = prepare_balanced_dataset(full_dataset, study)
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X_full, y_full)):
        ...
        rez_bias.append(calc_results_for_fold(X_full, y_full, train_index, test_index, "bias"))
