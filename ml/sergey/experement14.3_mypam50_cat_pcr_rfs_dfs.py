...
def full_dataset2pam50_raw_dataset(full_dataset, path = "./"):
    ...
    for _, row_full in full_dataset.iterrows():
        ...
        rez = []
        for t in types:
            to_sum = [row_full[row_pam50c["geneSymbol"]] * row_pam50c[t] for _,row_pam50c in pam50centroids.iterrows()]
            rez.append(np.nansum(to_sum))
        i_t_max = np.argmax(rez)
        for i,t in enumerate(types):
            centroids[t].append(int(i == i_t_max))            
...
pam_dataset = full_dataset2pam50_raw_dataset(full_dataset)
