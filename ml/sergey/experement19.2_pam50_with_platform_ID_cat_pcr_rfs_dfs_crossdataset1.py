...
def add_platform_ID_field(dataset, path="./"):
    atreat_dataset = pd.read_csv(os.path.join(path, 'bcClinicalTable.csv'))
    return pd.merge(dataset, atreat_dataset[["platform_ID", "patient_ID"]])
...
def print_results_for_field(dataset, field):
    ...
    for test_study in sorted(all_studies):
        ...
        rez["full_logi"]    = calc_results_for_test_study(dataset, test_study, LogisticRegression(max_iter=1000))
        
        rez["notrea_xgboost"] = calc_results_for_test_study(notrea_dataset, test_study, XGBClassifier())
        rez["notrea_logi"]    = calc_results_for_test_study(notrea_dataset, test_study, LogisticRegression(max_iter=1000))
...
pam_types_cat_dataset = add_platform_ID_field(pam_types_cat_dataset)
