...
def main():
    opt = parse_args()
    cuda = True
    train = opt.eval_on_train

    skip_study = opt.skip_study
    if skip_study == -1:
        skip_study = None
    additional_columns = ['study_ID', 'pam_coincide', 'patient_ID']
    ...
    if hasattr(opt, 'disc_path') and opt.disc_path:
        ...
        size = train_set.features.shape[1] - len(additional_columns) # substract addional columns
        ...
        discriminator.eval()
        code = discriminator.compute_code(torch.as_tensor(test_set.features.drop(columns=additional_columns).to_numpy()).float()).argmax(dim=1)
        ...
        result = {'patient_ID': test_set.features.patient_ID, 'code': code}
        df = pandas.DataFrame.from_dict(result)
        save_path = 'code_{0}.csv'.format('train' if train else 'test')
        df.to_csv(save_path, index=False)
        print('codes saved to ', save_path)
    else:
