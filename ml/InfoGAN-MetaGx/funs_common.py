...
def drop_trea(full_dataset):
    return full_dataset.drop(columns=['chemo', 'hormone']) # 'radio','surgery',
...
def prepare_full_dataset(full_dataset, y_field = 'posOutcome'):
    ...
    X = full_dataset.drop(columns=['sample_name', 'alt_sample_name','recurrence', 'hormone', 'chemo', 'posOutcome']).to_numpy()
