...
def print_results_for_field(dataset, field):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X_full, y_full)):
        ...
        for order in print_order:
            ...
            output.write(str(order) + str(" "*(max_len_order - len(order))) + ": " + str(list_to_4g_str(rez[order][-1])) + "\n")
        print ("")
        output.write("\n")
        sys.stdout.flush()
    ...
    for order in print_order:
        ...
        output.write("==> " + str(order) + str(" "*(max_len_order - len(order))) + ": " + str(list_to_4g_str(np.mean(rez[order], axis=0))) + "\n")
...
for dataset in datasets:
    metagx_dataset = read_metagx_dataset_with_stuff(path+dataset, ['N', 'age_at_initial_pathologic_diagnosis', 'grade', 'sample_name'])
