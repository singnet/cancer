...
def print_results_for_field(dataset, field):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X_full, y_full)):
        ...
        for order in print_order:
            ...
            output.write(str(order) + str(" " * (max_len_order - len(order))) + ": " + str(
...
for dataset in datasets:
    metagx_dataset = read_metagx_dataset_with_stuff(path + dataset,
                                                    ['tumor_size', 'N', 'age_at_initial_pathologic_diagnosis', 'grade',
                                                     'sample_name'])
